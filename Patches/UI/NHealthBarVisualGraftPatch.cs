...
public static class NHealthBarVisualGraftPatch
{
    ...
    internal static void RefreshGraftOverlay(NHealthBar healthBar)
    {
    ...
    internal static void AfterForecastTouchup(NHealthBar healthBar)
    {
        var creature = healthBar._creature;
        if (creature.CurrentHp <= 0 || creature.ShowsInfiniteHp)
            return;
        ...
        if (UiStates[healthBar] is not { } state || state.Strip == null)
            return;
    ...
    private static void SyncHpBarToHitbox(NHealthBar healthBar, float widthMultiplier)
    {
        ...
        healthBar.SetHpBarContainerSizeWithOffsetsImmediately(new Vector2(targetX, hpBar.Size.Y));
